...
The ML module predicts continuous network-performance metrics such as **Download Speed, Upload Speed, Latency, and Jitter**. It uses a preprocessing pipeline that:

- extracts hour/day/month from `Timestamp`
- imputes missing values
- one-hot encodes categorical fields such as Location, Carrier, Band, Device Model and Network Type
- compares Linear Regression, Decision Tree, Random Forest and Gradient Boosting
- reports MAE, RMSE and R²
- provides a feature-importance chart for tree models
- provides an interactive prediction form

`Dropped Connection` is not recommended as the primary ML target because it is close to a random 50/50 outcome in the supplied dataset.
